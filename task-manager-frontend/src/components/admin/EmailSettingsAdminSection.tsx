...
export const EmailSettingsAdminSection: React.FC = () => {
  ...
  return <div className="space-y-4" data-testid="admin-email-settings">
    ...
    <section className="rounded-[16px] border border-[#dedede] bg-white p-4"><h3 className="font-semibold">Автоматические уведомления</h3><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
    ...
    <section className="rounded-[16px] border border-[#dedede] bg-white p-4"><h3 className="font-semibold">Шаблоны писем</h3><p className="mt-1 text-xs text-[#777]">Переменные: {'{{ticketNumber}}'}, {'{{title}}'}, {'{{description}}'}, {'{{priority}}'}, {'{{status}}'}, {'{{oldStatus}}'}, {'{{requesterName}}'}, {'{{assigneeName}}'}, {'{{comment}}'}, {'{{portalLink}}'}.</p><div className="mt-3 grid gap-4 xl:grid-cols-2">
      {(['created','comment','status','assigned','assignee'] as const).map((kind) => { const subject=`${kind}SubjectTemplate` as keyof EmailSettingsAdmin; const body=`${kind}BodyTemplate` as keyof EmailSettingsAdmin; const names={created:'Создание для заявителя',comment:'Ответ заявителю',status:'Статус для заявителя',assigned:'Назначение для заявителя',assignee:'Назначение для исполнителя'}; return <div key={kind} className="rounded-[12px] border border-[#e5e5e5] p-3"><p className="mb-2 text-sm font-medium">{names[kind]}</p><input className={input} value={String(draft[subject])} onChange={e=>set(subject,e.target.value as never)}/><textarea className="input mt-2 min-h-32 w-full" value={String(draft[body])} onChange={e=>set(body,e.target.value as never)}/></div>; })}
    </div></section>
