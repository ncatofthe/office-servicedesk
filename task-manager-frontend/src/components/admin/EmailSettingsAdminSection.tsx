...
export const EmailSettingsAdminSection: React.FC = () => {
  ...
  return <div className="space-y-4" data-testid="admin-email-settings">
    ...
    <section className="rounded-[16px] border border-[#dedede] bg-[#fafafa] p-4"><h3 className="font-semibold">Режимы работы</h3><div className="mt-3 grid gap-3 md:grid-cols-2">
      ...
      <Toggle label="Уведомлять заявителя" description="Главный выключатель автоматических писем о заявке." checked={draft.notificationsEnabled} onChange={(v)=>set('notificationsEnabled',v)}/>
    </div></section>
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="rounded-[16px] border border-[#dedede] bg-white p-4"><h3 className="font-semibold">Входящая почта (IMAP)</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Интервал проверки, мс"><input type="number" className={input} value={draft.intakePollIntervalMs} onChange={e=>set('intakePollIntervalMs',Number(e.target.value))}/></Field><Field label="Писем за цикл"><input type="number" className={input} value={draft.intakeMaxMessages} onChange={e=>set('intakeMaxMessages',Number(e.target.value))}/></Field>
    ...
    <section className="rounded-[16px] border border-[#dedede] bg-white p-4"><h3 className="font-semibold">Когда писать заявителю</h3><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
    ...
    <section className="rounded-[16px] border border-[#dedede] bg-white p-4"><h3 className="font-semibold">Шаблоны писем</h3><p className="mt-1 text-xs text-[#777]">Переменные: {'{{ticketNumber}}'}, {'{{title}}'}, {'{{status}}'}, {'{{oldStatus}}'}, {'{{requesterName}}'}, {'{{assigneeName}}'}, {'{{comment}}'}, {'{{portalLink}}'}.</p><div className="mt-3 grid gap-4 xl:grid-cols-2">
      {(['created','comment','status','assigned'] as const).map((kind) => { const subject=`${kind}SubjectTemplate` as keyof EmailSettingsAdmin; const body=`${kind}BodyTemplate` as keyof EmailSettingsAdmin; const names={created:'Создание',comment:'Ответ',status:'Статус',assigned:'Назначение'}; return <div key={kind} className="rounded-[12px] border border-[#e5e5e5] p-3"><p className="mb-2 text-sm font-medium">{names[kind]}</p><input className={input} value={String(draft[subject])} onChange={e=>set(subject,e.target.value as never)}/><textarea className="input mt-2 min-h-32 w-full" value={String(draft[body])} onChange={e=>set(body,e.target.value as never)}/></div>; })}
    </div></section>
